    ...
    for (f=0;f<nf;f++) {
        ...
        for (i=1;i<=MAXSAT;i++) {
            ...
            if (rtk->opt.modear!=ARMODE_INST&&reset) {
				rtk->ssat[i - 1].lock[f] = 0;// -rtk->opt.minlock;
            }
        }
        ...
        for (i=0;i<ns;i++) {
            ...
            rtk->x[j]=0.0;
			rtk->ssat[sat[i] - 1].lock[f] = 0;// -rtk->opt.minlock;
        }
        bias=zeros(ns,1);
                      ...
                      const prcopt_t *opt, double *y)
{
    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* iono-free linear combination */
        ...
        for (i=0;i<nf;i++) {
			...
			if (obs->L[i] != 0.0 && fabs(obs->L[i])<1.0E12) y[i] = obs->L[i] * lam[i] - r - dant[i];
			if (obs->P[i] != 0.0 && obs->P[i]<1.0E10) y[i + nf] = obs->P[i] - r - dant[i];
        }
    }
}
...
#else
                if (rtk->x[i]==0.0||!test_sys(rtk->ssat[i-k].sys,m)||
                    !rtk->ssat[i-k].vsat[f]||!rtk->ssat[i-k].half[f]) {//here  2017/02/21
#endif
                    continue;
                }
...
static void restparamb(rtk_t *rtk, const double *bias, int nb, double *xa, int *index0)
{
	int i, n, m, f, index[MAXSAT], nv = 0, t = 0, nf = NF(&rtk->opt);
...
static void restwlamb(rtk_t *rtk, double *bias, double *xa)
{
	int i, n, m, index[MAXSAT], index1[MAXSAT], nv = 0, q = 0, nf = NF(&rtk->opt);
	...
	for (m = 0; m < 4; m++) {
		...
		for (n = i = q = 0; i < MAXSAT; i++) {
			if (!test_sys(rtk->ssat[i].sys, m) || rtk->ssat[i].fix[0] != 2 || rtk->ssat[i].fix[1] != 2) {
				continue;
			}
			index[n++] = IB(i + 1, 0, &rtk->opt); index1[q++] = IB(i + 1, 1, &rtk->opt);
		}
		if (n < 2) continue;

		xa[index[0]] = rtk->x[index[0]] - rtk->x[index1[0]];
...
/* hold integer ambiguity ----------------------------------------------------*/
static void holdwlamb(rtk_t *rtk, const double *xa)
{
	double *v, *H, *R;
	int i, n, m, info, index[MAXSAT], index1[MAXSAT], nb = rtk->nx - rtk->na, nv = 0, q = 0, nf = NF(&rtk->opt);
	...
	for (m = 0; m < 4; m++)
	{
		for (n = i = q = 0; i < MAXSAT; i++) {
			...
			index[n++] = IB(i + 1, 0, &rtk->opt);
			index1[q++] = IB(i + 1, 1, &rtk->opt);
			rtk->ssat[i].fix[0] = 3; rtk->ssat[i].fix[1] = 3;/* hold */

		}
		/* constraint to fixed ambiguity */
		for (i = 1; i<n; i++) {
			v[nv] = (xa[index[0]] - xa[index[i]]) - (rtk->x[index[0]] - rtk->x[index1[0]] - rtk->x[index[i]] + rtk->x[index1[i]]);

			H[index[0] + nv*rtk->nx] = 1.0;
			H[index1[0] + nv*rtk->nx] = -1.0;
			H[index[i] + nv*rtk->nx] = -1.0;
			H[index1[i] + nv*rtk->nx] = 1.0;
			nv++;
		}
	}
	if (nv>0) {
...
static int iMax2Min(double *dArray, int iLength, int *iASortIndex)
{
	int i = 0;
	int j = 0;
	double fDataTmp = 0.0;
	double *fArrayTmp = new double[iLength];
	int iIndexTmp = -1;
	for (i = 0; i < iLength; i++)
	{
		iASortIndex[i] = i;
		fArrayTmp[i] = dArray[i];
	}
	for (i = 0; i < iLength; i++)
	{
		for (j = 1; j < iLength; j++)
		{
			if (fArrayTmp[j] > fArrayTmp[j - 1])
			{
				fDataTmp = fArrayTmp[j - 1];
				fArrayTmp[j - 1] = fArrayTmp[j];
				fArrayTmp[j] = fDataTmp;

				iIndexTmp = iASortIndex[j - 1];
				iASortIndex[j - 1] = iASortIndex[j];
				iASortIndex[j] = iIndexTmp;
			}
		}
	}
	delete[] fArrayTmp;
...
static int resamb_PART_ELE_LAMBDA(rtk_t *rtk, double *bias, double *xa)
{
	prcopt_t *opt = &rtk->opt;
	int i, j, ny, nb, info, nx = rtk->nx, na = rtk->na;
	double *D, *DP, *y, *Qy, *b, *db, *Qb, *Qab, *QQ, s[2];
	double elmaskar = rtk->opt.elmaskar, elevations[MAXSAT];
	int ele_flag = 0, ele_sortedindex[MAXSAT];
	...
	for (i = 0; i < MAXSAT; ++i)
	{
		elevations[i] = rtk->ssat[i].azel[1];
	}
	iMax2Min(elevations, MAXSAT, ele_sortedindex);
	for (i = 0; i < MAXSAT; ++i)
	{
		if (elevations[ele_sortedindex[i]] <= rtk->opt.elmaskar)
		{
			ele_flag = i;
			break;
		}
	...
	do 
	{
		...
		if (--ele_flag >= 0)
			rtk->opt.elmaskar = elevations[ele_sortedindex[ele_flag]];
		else
			break;
	} while ((!info ? (s[0] > 0 ? s[1]/s[0] < opt->thresar[0] : (s[0] == 0 ? 0 : 1)) : info) && nb >= 4);
	...
	if (!info) {
		...
		rtk->sol.ratio = s[0]>0 ? (float)(s[1] / s[0]) : (s[0]==0 ? 999.9f : 0.0f);
...
/* resolve integer ambiguity by LAMBDA ---------------------------------------
[1]Dai L, Eslinger D, Sharpe T. Innovative
orithms to improve long range RTK reliability and availability[C]
//Proceedings of the 2007 National Technical Meeting of the Institute of Navigation. 2007: 860-872.

*/
static int resamb_PART_EQA_LAMBDA(rtk_t *rtk, double *bias, double *xa)
{
	prcopt_t *opt = &rtk->opt;
	int i, j, ny, nb, info, nx = rtk->nx, na = rtk->na, n0, *index0, *index1;
	double *D, *DP, *y, *Qy, *b, *Qb, *Qab, *QQ, s[4];
	double *Qab1, *Qb1, *db1, *y1;
	...
	ny = na + nb; y = mat(ny, 1); Qy = mat(ny, ny); DP = mat(ny, nx); index0 = (int*)malloc(sizeof(int)*nb); index1 = (int*)malloc(sizeof(int)*nb);
	b = mat(nb, 2); Qb = mat(nb, nb); Qab = mat(na, nb);
	//printMat(D, nx, nx);
	...
	if (rtk->opt.ffrt == 1)
	{
		info = PARlambdaffrt(nb, 2, y + na, Qb, b, s, &n0, index0);
	}
	else
	{
		info = PARlambda(nb, 2, y + na, Qb, b, s, &n0, index0); s[2] = 1 / opt->thresar[0];
	}
	Qab1 = mat(na, n0); Qb1 = mat(n0, n0); db1 = mat(n0, 1); y1 = mat(n0, 1); QQ = mat(na, n0);
	if (!info) {
		//printMat(b,nb,2);
		...
		rtk->sol.ratio = s[0]>0 ? (float)(s[1] / s[0]) : 0.0f;
		if (rtk->sol.ratio>999.9) rtk->sol.ratio = 999.9f;
		for (i = 0; i<na; i++) for (j = 0; j<n0; j++) Qab1[i + j*na] = Qab[i + index0[j] * na];
		for (i = 0; i<n0; i++)
		{
			y1[i] = y[na + index0[i]];
			for (j = 0; j<n0; j++)
			{
				Qb1[i + j*n0] = Qb[index0[i] + index0[j] * nb];
			}
		}
		/* validation by popular ratio-test */
		if (s[0] <= 0.0 || s[1] / s[0] >= 1 / s[2]) {
			/* transform float to fixed solution (xa=xa-Qab*Qb\(b0-b)) */
			for (i = 0; i<na; i++) {
				rtk->xa[i] = rtk->x[i];
				for (j = 0; j<na; j++) rtk->Pa[i + j*na] = rtk->P[i + j*nx];
			...
			if (!matinv(Qb1, n0)) {
				matmul("NN", n0, 1, n0, 1.0, Qb1, y1, 0.0, db1);
				matmul("NN", na, 1, n0, -1.0, Qab1, db1, 1.0, rtk->xa);

				/* covariance of fixed solution (Qa=Qa-Qab*Qb^-1*Qab') */
				matmul("NN", na, n0, n0, 1.0, Qab1, Qb1, 0.0, QQ);
				matmul("NT", na, na, n0, -1.0, QQ, Qab1, 1.0, rtk->Pa);
				...
				for (i = 0; i<n0; i++)
					index1[index0[i]] = 0;
				/* restore single-differenced ambiguity */
				restparamb(rtk, bias, nb, xa, index1);
				//holdparamb(rtk,xa,index1);
			}
			else nb = 0;
		}
		else { /* validation failed */
	...
	free(D); free(y); free(Qy); free(DP);
	free(b); free(Qb); free(Qab); free(index0); free(index1);

	free(QQ); free(db1); free(Qb1); free(Qab1); free(y1);

	return nb; /* number of ambiguities */
}
/* resolve integer ambiguity by LAMBDA ---------------------------------------
[1]Hou Y, Verhagen S. Model and Data Driven Partial Ambiguity Resolution for Multi-Constellation GNSS[C]
//China Satellite Navigation Conference (CSNC) 2014 Proceedings: Volume II. Springer Berlin Heidelberg, 2014: 285-302.
[2] Verhagen S, Teunissen P J G, van der Marel H, et al. GNSS ambiguity resolution: which subset to fix[C]
//Proceedings of IGNSS Symposium, Sydney, Australia. 2011
[3]Wang J, Feng Y. Reliability of partial ambiguity fixing with multiple GNSS constellations[J].
Journal of geodesy, 2013, 87(1): 1-14.
*/
static int resamb_SRCPARLAMBDA(rtk_t *rtk, double *bias, double *xa)
{
	prcopt_t *opt = &rtk->opt;
	int i, j, k, ny, nb, info, nx = rtk->nx, na = rtk->na, n0[1], *index0, *index1;
	double *D, *DP, *y, *Qy, *Qb, *Qab, *QQ, s[3];
	double *Qaz, *db1, *Q1, *E1, *Z1, *Qpar, *Zpar, *Epar;
	...
	ny = na + nb; y = mat(ny, 1); Qy = mat(ny, ny); DP = mat(ny, nx); index0 = (int*)


		(sizeof(int)*nb); index1 = (int*)malloc(sizeof(int)*nb);
	...
	// printMat(Qb, nb, nb);
	trace(4, "N(0)="); tracemat(4, y + na, 1, nb, 10, 3);
	Z1 = mat(nb, nb); Q1 = mat(nb, nb); E1 = mat(nb, 2);
	/* lambda/mlambda integer least-square estimation */
	if (rtk->opt.ffrt == 1)
	{
		info = SRCPARlambdaffrt(nb, 2, y + na, Qb, E1, Z1, Q1, s, n0, 0.999);
	}
	else
	{
		info = SRCPARlambda(nb, 2, y + na, Qb, E1, Z1, Q1, s, n0, 0.999); s[2] = 1 / opt->thresar[0];
	}
	Zpar = mat(nb, n0[0]); Qpar = mat(n0[0], n0[0]); Epar = mat(n0[0], 1); QQ = mat(na, n0[0]); Qaz = mat(na, n0[0]);	db1 = mat(n0[0], 1);
	if (!info) {
		...
		for (i = 0; i<n0[0]; i++)
		{
			Epar[i] = E1[i];
			for (j = 0; j<n0[0]; j++)
				Qpar[j + i*n0[0]] = Q1[j + i*n0[0]];
			for (k = 0; k<nb; k++)
				Zpar[i + k*n0[0]] = Z1[i + k*n0[0]];
		}

		matmul("NN", na, n0[0], nb, 1.0, Qab, Zpar, 0.0, Qaz);
		//printMat(Qaz,na,n0[0]);
		//printMat(Zpar,5,30);printMat(Qpar,5,5);printMat(Epar,1,5);
		...
		if (s[0] <= 0.0 || s[1] / s[0] >= 1 / s[2]) {
			...
			if (!matinv(Qpar, n0[0])) {
				matmul("NN", n0[0], 1, n0[0], 1.0, Qpar, Epar, 0.0, db1);
				matmul("NN", na, 1, n0[0], -1.0, Qaz, db1, 1.0, rtk->xa);
				// matmul("NN",n0[0],1,n0[0], 1.0,Qb1 ,y1,0.0,db1);
				// matmul("NN",na,1,n0[0],-1.0,Qab1,db1  ,1.0,rtk->xa);

				/* covariance of fixed solution (Qa=Qa-Qab*Qb^-1*Qab') */
				matmul("NN", na, n0[0], n0[0], 1.0, Qaz, Qpar, 0.0, QQ);
				matmul("NT", na, na, n0[0], -1.0, QQ, Qaz, 1.0, rtk->Pa);
	...
	free(db1);  free(Qaz); free(Q1); free(E1); free(Z1); free(Qpar); free(Epar); free(Zpar);

	return nb; /* number of ambiguities */
}
/* resolve integer ambiguity by LAMBDA 宽窄巷---------------------------------------
ref[1]鄢子平,丁乐乐,黄恩兴,于兴旺. 网络RTK参考站间模糊度固定新方法[J]. 武汉大学学报•信息科学版
ref[2]Li J, Yang Y, Xu J, et al. GNSS multi-carrier fast partial ambiguity resolution strategy
tested with real BDS/GPS dual-and triple-frequency observations[J]. GPS Solutions, 1-9.
*/
static int resamb_WNLAMBDA(rtk_t *rtk, double *bias, double *xa)
{
	prcopt_t *opt = &rtk->opt;
	int i, j, ny, nb, nw, info, nx = rtk->nx, na = rtk->na;
	double *xb, *D, *DP, *y, *af, *daf, *nf, *Qy, *b, *b1, *db, *Qb, *Qab, *QQ, *Qw, *Q11, *Q12, *Q22, *Q21, *Qw2, *Q2w, *Q22c, s[4];
	trace(3, "resamb_LAMBDA : nx=%d\n", nx);
	...
	ny = na + nb; nw = nb / 2; y = mat(ny, 1); af = mat(nw, 1); daf = mat(nw, 1); nf = mat(nw, 1); Qy = mat(ny, ny); DP = mat(ny, nx);
	b = mat(nw, 2); b1 = mat(nw, 2); db = mat(nb, 1); Qb = mat(nb, nb); Qab = mat(na, nb); QQ = mat(na, nb); xb = mat(rtk->nx, 1);
	Q11 = mat(nw, nw); Q12 = mat(nw, nw); Q21 = mat(nw, nw); Q22 = mat(nw, nw); Qw = mat(nw, nw); Q22c = mat(nw, nw);
	Q2w = mat(nw, nw); Qw2 = mat(nw, nw);
	//printMat(D, nx, nx);
	...
	matmul("TN", ny, nx, nx, 1.0, D, rtk->P, 0.0, DP);
	matmul("NN", ny, ny, nx, 1.0, DP, D, 0.0, Qy);
	for (i = 0; i<nw; i++) af[i] = y[na + i] - y[na + nw + i];

	/* phase-bias covariance (Qb) and real-parameters to bias covariance (Qab) */
	for (i = 0; i<nb; i++) for (j = 0; j<nb; j++) Qb[i + j*nb] = Qy[na + i + (na + j)*ny];
	for (i = 0; i<na; i++) for (j = 0; j<nb; j++) Qab[i + j*na] = Qy[i + (na + j)*ny];
	for (i = 0; i<nw; i++) for (j = 0; j<nw; j++)
	{
		Q11[i + j*nw] = Qb[i + j*nb]; Q12[i + j*nw] = Qb[nw + i + j*nb]; Q21[i + j*nw] = Qb[i + (nw + j)*nb]; Q22[i + j*nw] = Qb[i + nw + (nw + j)*nb];
	}
	//printMat(Qb, nb, nb);
	trace(4, "N(0)="); tracemat(4, y + na, 1, nb, 10, 3);
	for (i = 0; i<nw; i++) for (j = 0; j<nw; j++)
	{
		Qw[i + j*nw] = Q11[i + j*nw] - Q12[i + j*nw] - Q21[i + j*nw] + Q22[i + j*nw];
		Qw2[i + j*nw] = Q12[i + j*nw] - Q22[i + j*nw]; Q2w[i + j*nw] = Q21[i + j*nw] - Q22[i + j*nw];
	}
	/* lambda/mlambda integer least-square estimation */
	if (rtk->opt.ffrt == 1)
	{
		info = lambdaffrt(nw, 2, af, Qw, b, s);
	}
	else
	{
		info = lambda(nw, 2, af, Qw, b, s); s[2] = 1 / opt->thresar[0];
	}
	if (!info) {
		//trace(4,"N(1)="); tracemat(4,b   ,1,nb,10,3);
		//trace(4,"N(2)="); tracemat(4,b+nb,1,nb,10,3);
		...
		//printMat(Qw, nw, nw);
		/* validation by popular ratio-test */
		if (s[0] <= 0.0 || s[1] / s[0] >= 1 / s[2]) {    //固定窄巷----------
			for (i = 0; i<nw; i++){ af[i] -= b[i]; nf[i] = y[na + nw + i]; }
			if (!matinv(Qw, nw))
				//宽相固定后修正窄巷的浮点解模糊度
			{   //printMat(Q22, nw, nw);
				matmul("NN", nw, 1, nw, 1.0, Qw, af, 0.0, daf);
				matmul("NN", nw, 1, nw, -1.0, Qw2, daf, 1.0, nf);
				matmul("NN", nw, nw, nw, 1.0, Qw, Q2w, 0.0, Q22c);
				matmul("NN", nw, nw, nw, -1.0, Qw2, Q22c, 1.0, Q22);
				//				printMat(Q22, nw, nw);
				if (rtk->opt.ffrt == 1)
				{
					info = lambdaffrt(nw, 2, nf, Q22, b1, s); trace(6, "ratio=%5.2f,%6.4f,%d\n", s[1] / s[0], s[3], nb);;
				}
				else
				{
					info = lambda(nw, 2, nf, Q22, b1, s); s[2] = 1 / opt->thresar[0];
				}
				if (!info) {
					rtk->sol.ratio = s[0]>0 ? (float)(s[1] / s[0]) : 0.0f;
					if (rtk->sol.ratio>999.9) rtk->sol.ratio = 999.9f;
					if (s[0] <= 0.0 || s[1] / s[0] >= 1 / s[2]) {
						...
						for (i = 0; i<nw; i++) {
							bias[i] = b[i] + b1[i]; bias[i + nw] = b1[i];
							y[na + i] -= b[i] + b1[i]; y[na + nw + i] -= b1[i];
						}
						if (!matinv(Qb, nb)) {
							matmul("NN", nb, 1, nb, 1.0, Qb, y + na, 0.0, db);
							matmul("NN", na, 1, nb, -1.0, Qab, db, 1.0, rtk->xa);
	...
	free(D); free(y); free(Qy); free(DP); free(af); free(daf); free(nf); free(b1); free(Qw); free(Qw2); free(xb);
	free(b); free(db); free(Qb); free(Qab); free(QQ); free(Q11); free(Q12); free(Q21); free(Q22); free(Q22c); free(Q2w);
                  ...
                  const nav_t *nav)
{
	...
	int i, j, f, n = nu + nr, ns, ny, nv, sat[MAXSAT], iu[MAXSAT], ir[MAXSAT], niter, arinfo;
    ...
    /* resolve integer ambiguity by LAMBDA */
	else if (stat != SOLQ_NONE&&rtk->opt.modear > 0) 
	{
		switch (rtk->opt.modepar)
		{
		case PARMODE_OFF:
			arinfo = resamb_PART_ELE_LAMBDA(rtk, bias, xa);
			//arinfo = resamb_LAMBDA(rtk, bias, xa);
			break;
		case PARMODE_WN:
			arinfo = resamb_WNLAMBDA(rtk, bias, xa);
			break;
		case PARMODE_FS:
			arinfo = resamb_PART_EQA_LAMBDA(rtk, bias, xa);
			break;
		case PARMODE_SRC:
			arinfo = resamb_SRCPARLAMBDA(rtk, bias, xa);
			break;
		}
		if (arinfo > 1)
		{
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    if (opt->mode==PMODE_MOVEB) { /*  moving baseline */
		...
		//printf("%d ", obs[0].time.time); printf(" %d\n", obs[nu].time.time);
