...
/*copy data*/
static void obscopy(obs_t *obsc, obs_t *obs)
{
	int i;
	if (!obsc || !obs) return;

	memset(obsc->data, 0, sizeof(obsd_t)*MAXOBS);
	obsc->n = 0;
	// copy obs.
	for (i = 0; i < obs->n; ++i){
		obsc->data[obsc->n++] = obs->data[i];
	}
	// clear old data.
...
static int decode_head1104(rtcm_t *rtcm, int *sync)
{
	...
	// convert BDT to GPST.
	tow += 14;
	if (tow >= 604800.0) tow -= 604800.0;

	adjweek(rtcm, tow);

	trace(4, "decode_head1104: time=%s nsat=%d sync=%d\n", time_str(rtcm->time, 2),
		nsat, *sync);
...
/* decode type 1104: extended RTCM3.x B1&B2&B3 bds rtk observables -----------*/
static int decode_type1104(rtcm_t *rtcm)
{
	const int B1codes[] = { CODE_L2I, CODE_L2Q };
	const int B2codes[] = { CODE_L7I, CODE_L7Q };
	const int B3codes[] = { CODE_L6I, CODE_L6Q };
	double pr1, pr2, pr3, cnr1, cnr2, cnr3, tt, cp1, cp2, cp3;
	int i = 24 + 64, j, index, nsat, sync, prn, sat, code1, code2, code3, ppr1, ppr2, ppr3, ret = 0;
	int lock1, lock2, lock3, amb1, amb2, amb3, sys = SYS_CMP;
	int b123f;
	...
	if (i + 3>rtcm->len * 8) return -1;
	b123f = getbitu(rtcm->buff, i, 3); i += 3;

	for (j = 0; j < nsat&&rtcm->obs.n < MAXOBS&&i + 213 <= rtcm->len * 8; j++) {
		prn = getbitu(rtcm->buff, i, 6); i += 6;
		code1 = getbitu(rtcm->buff, i, 2); i += 2; // B1
		pr1 = getbitu(rtcm->buff, i, 24); i += 24;
		ppr1 = getbits(rtcm->buff, i, 20); i += 20;
		lock1 = getbitu(rtcm->buff, i, 7); i += 7;
		amb1 = getbitu(rtcm->buff, i, 8); i += 8;
		cnr1 = getbitu(rtcm->buff, i, 8); i += 8;
		code2 = getbitu(rtcm->buff, i, 2); i += 2; // B2
		pr2 = getbits(rtcm->buff, i, 24); i += 24;
		ppr2 = getbits(rtcm->buff, i, 20); i += 20;
		lock2 = getbitu(rtcm->buff, i, 7); i += 7;
		amb2 = getbitu(rtcm->buff, i, 8); i += 8;
		cnr2 = getbitu(rtcm->buff, i, 8); i += 8;
		code3 = getbitu(rtcm->buff, i, 2); i += 2; // B3
		pr3 = getbits(rtcm->buff, i, 24); i += 24;
		...
		if (!(sat = satno(sys, prn))) {
			trace(2, "rtcm3 1104 satellite number error: sys=%d prn=%d\n", sys, prn);
			continue;
		}
		tt = timediff(rtcm->obs.data[0].time, rtcm->time);
		//if (rtcm->obs.data[0].time.time&&
		//	fabs(tt) > 1E-9){
		//	ret = 1;
		//	//printf("write1, %d\n", rtcm->obs.n);
		//	obscopy(&rtcm->obsc, &rtcm->obs);
		//}
		if ((index = obsindex(&rtcm->obs, rtcm->time, sat)) < 0) continue;
		// B1
		pr1 = pr1*0.02 + amb1*PRUNIT_BDS;
		if (ppr1 != (int)0xFFF80000) {
			rtcm->obs.data[index].P[0] = pr1;
			cp1 = adjcp(rtcm, sat, 0, ppr1*0.0005 / lam_carr[6]);
			rtcm->obs.data[index].L[0] = pr1 / lam_carr[6] + cp1;
		}
		rtcm->obs.data[index].LLI[0] = lossoflock(rtcm, sat, 0, lock1);
		rtcm->obs.data[index].SNR[0] = snratio(cnr1*0.25);
		rtcm->obs.data[index].code[0] = code1 ? CODE_L2I : CODE_L2I;

		// B2
		pr2 = pr2*0.02 + amb2*PRUNIT_BDS;
		if (ppr2 != (int)0xFFF80000) {
			rtcm->obs.data[index].P[1] = pr2;
			cp2 = adjcp(rtcm, sat, 0, ppr2*0.0005 / lam_carr[7]);
			rtcm->obs.data[index].L[1] = pr2 / lam_carr[7] + cp2;
		}
		rtcm->obs.data[index].LLI[1] = lossoflock(rtcm, sat, 0, lock2);
		rtcm->obs.data[index].SNR[1] = snratio(cnr2*0.25);
		rtcm->obs.data[index].code[1] = code2 ? CODE_L7I : CODE_L7I;

		// B3
		pr3 = pr3*0.02 + amb3*PRUNIT_BDS;
		if (ppr3 != (int)0xFFF80000) {
			rtcm->obs.data[index].P[2] = pr3;
			cp3 = adjcp(rtcm, sat, 0, ppr3*0.0005 / lam_carr[8]);
			rtcm->obs.data[index].L[2] = pr3 / lam_carr[8] + cp3;
		}
		rtcm->obs.data[index].LLI[2] = lossoflock(rtcm, sat, 0, lock3);
		rtcm->obs.data[index].SNR[2] = snratio(cnr3*0.25);
		rtcm->obs.data[index].code[2] = code3 ? CODE_L6I : CODE_L6I;
	}
	//if (!sync)
	//{
	//	obscopy(&rtcm->obsc, &rtcm->obs);
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
    ...
    switch (type) {
        ...
        case 9104: ret=decode_msm4(rtcm,SYS_SBS); break;
