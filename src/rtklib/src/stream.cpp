...
static serial_t *openserial(const char *path, int mode, char *msg)
{
    ...
    char *p=nullptr,parity='N',dev[128],port[128],fctr[64]="",path_tcp[32],msg_tcp[128];
	char path_[1024];
	strcpy(path_, path);
#ifdef WIN32
	...
	//MultiByteToWideChar(CP_ACP, 0, dev, strlen(dev) + 1, wszClassName,
	//	sizeof(wszClassName) / sizeof(wszClassName[0]));
	//serial->dev = CreateFile(wszClassName, rw, 0, 0, OPEN_EXISTING, 0, NULL);
	...
	//WCHAR wszClassName[1024];
	//memset(wszClassName, 0, sizeof(wszClassName));
	//MultiByteToWideChar(CP_ACP, 0, dcb, strlen(dcb) + 1, wszClassName,
	//	sizeof(wszClassName) / sizeof(wszClassName[0]));
	//if (!BuildCommDCB(wszClassName, &cc.dcb)) {
