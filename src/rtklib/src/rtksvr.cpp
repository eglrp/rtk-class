...
#endif
	{
		...
		for (cycle = 0; svr->state; cycle++) {
			...
			for (i = 0; i < fobs[0]; i++) { /* for each rover observation data */
				...
				if (0/*!strstr(svr->rtk.opt.pppopt, "-DIS_FCB")*/) {
				...
				if (svr->rtk.sol.stat != SOLQ_NONE) {
					...
					///unsigned char buff[1024];
					//outsols(buff, &svr->rtk.sol, svr->rtk.rb, svr->solopt);
					//printf("%s", buff);
					/* write solution */
					writesol(svr, i);
				}
	...
	*-----------------------------------------------------------------------------*/
		...
		solopt_t *solopt, stream_t *moni, char *errmsg)
	{
		...
		for (i = 0; i < 3; i++) { /* input/log streams */
			...
			if (cmds_periodic)
				strcpy(svr->cmds_periodic[i], cmds_periodic[i]);
			else
				strcpy(svr->cmds_periodic[i], "");
