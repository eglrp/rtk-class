...
#define FTP_RETRY   3               /* ftp number of retry */
	...
	static void remot2local(const char *remot, const char *dir, char *local)
	{
		...
		if (strcpy(p, strrchr(remot, '/'))) p++; else p = (char *)remot;
	...
	static int exist_file(const char *local)
	{
		...
		MultiByteToWideChar(CP_ACP, 0, local, strlen(local) + 1, wszClassName,
			sizeof(wszClassName) / sizeof(wszClassName[0]));
		DWORD stat = GetFileAttributes(wszClassName);
	...
	static int mkdir_r(const char *dir)
	{
		...
		WCHAR wszClassName[1024];
		memset(wszClassName, 0, sizeof(wszClassName));
		MultiByteToWideChar(CP_ACP, 0, pdir, strlen(pdir) + 1, wszClassName,
			sizeof(wszClassName) / sizeof(wszClassName[0]));
		if ((p = strrchr(pdir, FILEPATHSEP))) {
			*p = '\0';
			h = FindFirstFile(wszClassName, &data);
		...
		//WCHAR wszClassName[1024];
		memset(wszClassName, 0, sizeof(wszClassName));
		MultiByteToWideChar(CP_ACP, 0, dir, strlen(dir) + 1, wszClassName,
			sizeof(wszClassName) / sizeof(wszClassName[0]));
		if (CreateDirectory(wszClassName, NULL) ||
